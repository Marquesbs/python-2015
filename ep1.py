...
for k in txtB:
    if len(k) == 5 and k[-1] not in 'ftc' and k[0] not in 'ftc':
        v = v + 1
    if len(k) == 5 and k[-1] not in 'ftc':
        vpp = vpp + 1

print('Verbos: ', v)
print('Verbos 1a pessoa: ', vpp)

#Questão D

essealfabetoretardado = "ftczmbshjpnwlrxkqvdg"
alfabetonormal = "abcdefghijklmnopqrst"

def textodoido (texto):
    
    texto = " ".join(texto)
    textoindo = texto.maketrans(essealfabetoretardado, alfabetonormal)
    textodevolta = texto.maketrans(alfabetonormal, essealfabetoretardado)
    texto = texto.translate(textoindo).split()
    texto.sort()
    texto = " ".join(texto).translate(textodevolta).split()

    print(texto)
    return texto

print ('texto B ordenado: ')
textodoido(txtB)
